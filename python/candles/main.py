...
def export_candles(json_file, output_file="chart.png"):
    ...
    # Map our fields: "time", "open", "high", "low", "close", "volume"
    df.rename(columns={
        "time": "Date",
        "open": "Open",
        "high": "High",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Smart Money Concepts chart from OHLC JSON.")
    parser.add_argument("json_file", help="Path to the input JSON file (list of candles with 'time', 'open', 'high', 'low', 'close', 'volume')")
    parser.add_argument("-output", default="chart.png", help="Output image file path (default: chart.png)")
    args = parser.parse_args()
    
    input_path = os.path.expanduser(args.json_file)
    output_path = os.path.expanduser(args.output)
    
    export_candles(input_path, output_path)